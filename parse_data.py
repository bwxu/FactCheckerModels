...
def get_glove_vectors(path):
    ...
    Outputs:
      embedding dictionary where every key is a word and maps to an embedding vector
    '''
    embeddings = {}
...
def get_input_data(path):
    '''
    Converts the train, eval, and test TSVs into arrays in memory that can be easily
    accessed.
...
def get_data(path):
    '''
    From the 2d list representation of the data TSVs generated by get_input_data,
    return lists representing the labels, sentences, subjects, party, and history
    for each data point.
...
def get_one_hot_vectors(list_of_values, length, mapping):
    ...
    Outputs:
      vectors: list of one hot values corresponding to the original list of values 
    '''
...
def clean_credit(labels, credit):
    '''
    Removes from the credit history the current label. It's necessary to do this 
    before using the credit metadata as mentioned in the original paper; otherwise,
    there are many credit histories with only 1 data point, which the credit 
    metadata will directly map to.

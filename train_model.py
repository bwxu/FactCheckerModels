...
from parse_data import clean_credit, get_glove_vectors, get_data, \
...
def train_model():
    ...
    if var.USE_WORD2VEC:
        embeddings = KeyedVectors.load_word2vec_format(
            var.WORD2VEC_BIN_PATH, binary=True)
    else:
    ...
    y_train = to_categorical(np.asarray(
        [var.LABEL_MAPPING[label] for label in train_labels]))
    y_val = to_categorical(np.asarray(
        [var.LABEL_MAPPING[label] for label in val_labels]))
    ...
    for i in range(var.NUM_MODELS):
        ...
        if var.MODEL_TYPE == "CNN":
            ...
            model = parallel_cnn_bi_lstm_model(
                embedding_matrix,
                num_words,
                pooling=var.POOLING,
                subject=var.USE_SUBJECTS,
        ...
        checkpoint_file = os.path.join(
            var.FOLDER_NAME, str(i).zfill(2) + '_' + var.FILE_NAME)
